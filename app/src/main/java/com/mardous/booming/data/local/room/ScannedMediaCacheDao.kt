...
interface ScannedMediaCacheDao {
     ...
     */
    @Query("SELECT * FROM scanned_media_cache WHERE is_valid = 1 ORDER BY title ASC")
     ...
     */
    @Query("SELECT * FROM scanned_media_cache WHERE file_path = :path LIMIT 1")
     ...
     */
    @Query("SELECT * FROM scanned_media_cache WHERE media_store_id = :media_store_id LIMIT 1")
    suspend fun getByMediaStoreId(media_store_id: Long): ScannedMediaCache?

    /**
     * Obtiene archivos modificados desde un timestamp.
     * Útil para escaneo incremental.
     */
    @Query("SELECT * FROM scanned_media_cache WHERE last_modified > :timestamp AND is_valid = 1")
     ...
     */
    @Query("SELECT COUNT(*) FROM scanned_media_cache WHERE is_valid = 1")
     ...
     */
    @Query("UPDATE scanned_media_cache SET is_valid = 0 WHERE file_path = :path")
    suspend fun invalidate(path: String)

    /**
     * Elimina permanentemente un archivo del cache.
     */
    @Query("DELETE FROM scanned_media_cache WHERE file_path = :path")
     ...
     */
    @Query("DELETE FROM scanned_media_cache WHERE is_valid = 0 AND scan_timestamp < :olderThan")
     ...
     */
    @Query("SELECT file_path FROM scanned_media_cache WHERE is_valid = 1")
    suspend fun getAllValidPaths(): List<String>

    /**
     * Invalida múltiples archivos por sus paths.
     */
    @Query("UPDATE scanned_media_cache SET is_valid = 0 WHERE file_path IN (:paths)")
